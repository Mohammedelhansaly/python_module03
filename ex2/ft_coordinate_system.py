...
def parse_cordinates(cordinate_str: str) -> tuple:
    """Parse a string of coordinates in the format 'x,y,z' into a tuple of
       integers."""
    try:
...
def cordinate_system() -> None:
    ...
    print(f"Distance between {point1} and {point2}:"
          f" {calculate_distance(point1, point2):.2f}\n")

    point3 = "3,4,0"
    point3_coords = parse_cordinates(point3)
    print(f"Parsing coordinates: {point3}")
    print(f"Parsed position: {point3_coords}")
    print(f"Distance between {point1} and {point3_coords}: "
          f"{calculate_distance(point1, point3_coords):.2f}\n")
